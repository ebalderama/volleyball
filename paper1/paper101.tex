...
We develop a probit regression model for estimating $\rho_i = \P(Y_i = 1)$, the probability of winning point $i$, for $i \in \calD$. 


\beq
  \Phi^{-1}(\Hat\rho_i) = \alpha_0 + \bbeta\bfX + Z_i,
\eeq
where $\alpha_0$ is an intercept term, $\bfX$ is an $n \times p$ matrix of predictors with corresponding coefficient vector $\bbeta$, and $Z_i$ is a conditional game-flow random effect that depends on $i \cond i-1, i-2, \dots$.
